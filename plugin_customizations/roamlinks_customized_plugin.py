...
class BackLinksReplacer:
    # Replaces '[BACKLINKS]' in any document with list of roam-style links that are pointing to that file
    def __init__(self, base_docs_url, page_url):
        self.base_docs_url = base_docs_url
        self.page_url = page_url

    def __call__(self):
        file_name = re.sub(r'^.+/','',self.page_url)
        out = []
        for root, dirs, files in os.walk(self.base_docs_url):
            for name in files:
                full_path = os.path.join(root, name )
                roam_name = "[[" + file_name.replace(".md","") + "]]"
                with open(full_path, "r") as f:
                    data = f.read()
                    if roam_name in data:
                        # If we have a match, create the relative path from linker to the link
                        out.append("- [[{}]]".format(name.replace(".md","")) )
                    f.close()
        out.sort()
        retval = "\n".join(out)
...
class RoamLinksPlugin(BasePlugin):
    def on_page_markdown(self,
        ...
        # replace backlinks
        markdown = re.sub(r'\[BACKLINKS\]',BackLinksReplacer(base_docs_url, page_url)(),markdown)
